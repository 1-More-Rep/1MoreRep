...
export default async function ArchivedRoutinesPage() {
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 'var(--gap)' }}>
      ...
      <h1 style={{ fontSize: 24, fontWeight: 700, letterSpacing: '-.02em', margin: 0 }}>Archived routines</h1>
      <SectionLabel>Restore a routine to bring it back to your list.</SectionLabel>

      {archived.length === 0 && (
        <Card soft><span style={{ color: 'var(--text-3)' }}>Nothing archived.</span></Card>
      )}
        ...
        <Card key={r.id} style={{ display: 'flex', alignItems: 'center', gap: 12, flexWrap: 'wrap' }}>
          <div style={{ flex: '1 1 200px', minWidth: 0 }}>
            ...
            <div style={{ fontSize: 12.5, color: 'var(--text-3)', marginTop: 3 }}>
              <Mono>{r._count.items}</Mono> exercises
            </div>
          </div>
