...
export default async function AdminAuditPage() {
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 'var(--gap)' }}>
      <h1 style={{ fontSize: 22, fontWeight: 700, margin: 0 }}>{t('auditLogTitle')}</h1>
      <Card pad={false}>
        {events.length === 0 && <div style={{ padding: 'var(--pad)', color: 'var(--text-3)' }}>{t('noEvents')}</div>}
        {events.map((e, i) => (
          <div
            key={e.id}
            style={{
          ...
          >
            ...
            <span style={{ color: 'var(--text-2)' }}>{e.actor ? e.actor.displayName : t('systemActor')}</span>
